...
| :--- | :--- | :--- | :--- | :--- |
| `polylang` | Polylang | Active | Multi-language plugin no longer needed; site content is single-language (Arabic) with 0 English posts & 1 discardable draft page | Safe removal; default language set to Arabic and Polylang deactivated |
...
4. **Single Language Strategy**: Polylang will be safely deactivated and single language mode (Arabic) confirmed across core site settings.
...
- **Deliverables**:
  - Safely remove/deactivate `polylang` and `polylang-theme-strings` plugins and purge remaining English draft content.
...
| **Task 4: Polylang Cleanup & Runtime Transition** | ~20,000 | ~5,000 | ~25,000 | ~$0.14 |
